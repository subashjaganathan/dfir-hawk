...
public static class Db
{
    ...
    private const string Schema = """
        ...
        CREATE TABLE IF NOT EXISTS memory_modules (
            -- ldrmodules: a DLL missing from a PEB list (esp. in_load=0) is
            -- unlinked/hidden = injection or hollowing.
            id INTEGER PRIMARY KEY,
            pid INTEGER, process TEXT, base TEXT,
            in_load INTEGER, in_init INTEGER, in_mem INTEGER, mapped_path TEXT
        );
        CREATE TABLE IF NOT EXISTS memory_hooks (
            id INTEGER PRIMARY KEY,
            kind TEXT,                     -- ssdt | callback
            idx INTEGER, address TEXT, module TEXT, symbol TEXT, detail TEXT
        );
        CREATE TABLE IF NOT EXISTS memory_handles (
            -- opt-in (--handles); high volume, investigative reference only.
            id INTEGER PRIMARY KEY,
            pid INTEGER, process TEXT, handle_type TEXT, granted_access TEXT, name TEXT
        );
