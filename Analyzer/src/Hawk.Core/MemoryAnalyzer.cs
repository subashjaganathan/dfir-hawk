...
public static class MemoryAnalyzer
{
    ...
    public static int Analyze(SqliteConnection conn, string imagePath, string? volOverride = null,
        bool includeHandles = false, Action<string>? progress = null)
    {
        ...
        var ldr = RunPlugin(vol.Value, imagePath, "windows.ldrmodules", progress);
        var ssdt = RunPlugin(vol.Value, imagePath, "windows.ssdt", progress);
        var callbacks = RunPlugin(vol.Value, imagePath, "windows.callbacks", progress);
        var handles = includeHandles ? RunPlugin(vol.Value, imagePath, "windows.handles", progress) : null;

        var n = IngestProcesses(conn, pslist, psscan, progress)
              + IngestMalfind(conn, malfind, progress)
              + IngestNetscan(conn, netscan, progress)
              + IngestLdrModules(conn, ldr, progress)
              + IngestHooks(conn, ssdt, callbacks, progress)
              + IngestHandles(conn, handles, progress);
    ...
    /// <summary>ldrmodules: a DLL unlinked from a PEB list (esp. InLoad=false) is a
    /// classic injection/hollowing signal. Stores all; flags the unlinked ones.</summary>
    private static int IngestLdrModules(SqliteConnection conn, List<Dictionary<string, JsonElement>>? rows, Action<string>? progress)
    {
        if (rows is null) return 0;
        var findings = new List<(string, string, string, string, string?)>();
        using (var tx = conn.BeginTransaction())
        using (var cmd = conn.CreateCommand())
        {
            cmd.CommandText = "INSERT INTO memory_modules (pid,process,base,in_load,in_init,in_mem,mapped_path) VALUES ($pid,$p,$b,$il,$ii,$im,$mp)";
            var pPid = P(cmd, "$pid"); var pP = P(cmd, "$p"); var pB = P(cmd, "$b");
            var pIl = P(cmd, "$il"); var pIi = P(cmd, "$ii"); var pIm = P(cmd, "$im"); var pMp = P(cmd, "$mp");
            foreach (var r in rows)
            {
                var pid = Long(r, "PID"); var proc = Str(r, "Process"); var mapped = Str(r, "MappedPath");
                bool inLoad = Bool(r, "InLoad"), inInit = Bool(r, "InInit"), inMem = Bool(r, "InMem");
                pPid.Value = (object?)pid ?? DBNull.Value; pP.Value = (object?)proc ?? DBNull.Value;
                pB.Value = (object?)Str(r, "Base") ?? DBNull.Value;
                pIl.Value = inLoad ? 1 : 0; pIi.Value = inInit ? 1 : 0; pIm.Value = inMem ? 1 : 0;
                pMp.Value = (object?)mapped ?? DBNull.Value;
                cmd.ExecuteNonQuery();
                // Unlinked from the load-order list, but backed by a real mapped file = hidden/injected DLL.
                if (!inLoad && !string.IsNullOrWhiteSpace(mapped))
                    findings.Add(("memory-unlinked-module", "high",
                        $"Unlinked module in {proc ?? "?"} (PID {pid}): {System.IO.Path.GetFileName(mapped)}",
                        $"missing from PEB load list (InLoad=false, InInit={inInit}, InMem={inMem}); path={mapped}", null));
            }
            tx.Commit();
        }
        InsertFindings(conn, findings, "memory_modules", "T1055.001 | Process Injection: DLL Injection");
        return rows.Count;
    }

    /// <summary>SSDT + kernel callbacks. SSDT entries pointing outside the kernel
    /// images are hooks (rootkit); callbacks are stored for review.</summary>
    private static int IngestHooks(SqliteConnection conn, List<Dictionary<string, JsonElement>>? ssdt,
        List<Dictionary<string, JsonElement>>? callbacks, Action<string>? progress)
    {
        string[] kernelMods = ["ntoskrnl.exe", "ntkrnlpa.exe", "ntkrnlmp.exe", "ntkrpamp.exe", "win32k.sys", "win32kbase.sys", "win32kfull.sys"];
        var findings = new List<(string, string, string, string, string?)>();
        var total = 0;
        using (var tx = conn.BeginTransaction())
        using (var cmd = conn.CreateCommand())
        {
            cmd.CommandText = "INSERT INTO memory_hooks (kind,idx,address,module,symbol,detail) VALUES ($k,$i,$a,$m,$s,$d)";
            var pK = P(cmd, "$k"); var pI = P(cmd, "$i"); var pA = P(cmd, "$a"); var pM = P(cmd, "$m"); var pS = P(cmd, "$s"); var pD = P(cmd, "$d");
            foreach (var r in ssdt ?? new())
            {
                var mod = Str(r, "Module") ?? "";
                pK.Value = "ssdt"; pI.Value = (object?)Long(r, "Index") ?? DBNull.Value; pA.Value = (object?)Str(r, "Address") ?? DBNull.Value;
                pM.Value = (object?)mod ?? DBNull.Value; pS.Value = (object?)Str(r, "Symbol") ?? DBNull.Value; pD.Value = DBNull.Value;
                cmd.ExecuteNonQuery(); total++;
                if (mod.Length > 0 && !kernelMods.Any(k => mod.Equals(k, StringComparison.OrdinalIgnoreCase)))
                    findings.Add(("memory-ssdt-hook", "high",
                        $"SSDT entry hooked outside the kernel: index {Str(r, "Index")}",
                        $"resolves to module '{mod}' (symbol={Str(r, "Symbol")}) - expected ntoskrnl/win32k", null));
            }
            foreach (var r in callbacks ?? new())
            {
                pK.Value = "callback"; pI.Value = DBNull.Value; pA.Value = (object?)Str(r, "Callback") ?? DBNull.Value;
                pM.Value = (object?)Str(r, "Module") ?? DBNull.Value; pS.Value = (object?)Str(r, "Symbol") ?? DBNull.Value;
                pD.Value = (object?)(Str(r, "Type") ?? Str(r, "Detail")) ?? DBNull.Value;
                cmd.ExecuteNonQuery(); total++;
            }
            tx.Commit();
        }
        InsertFindings(conn, findings, "memory_hooks", "T1014 | Rootkit (SSDT hook)");
        return total;
    }

    /// <summary>Handles - high volume, investigative reference only (opt-in). No findings.</summary>
    private static int IngestHandles(SqliteConnection conn, List<Dictionary<string, JsonElement>>? rows, Action<string>? progress)
    {
        if (rows is null) return 0;
        using var tx = conn.BeginTransaction();
        using var cmd = conn.CreateCommand();
        cmd.CommandText = "INSERT INTO memory_handles (pid,process,handle_type,granted_access,name) VALUES ($pid,$p,$t,$g,$n)";
        var pPid = P(cmd, "$pid"); var pP = P(cmd, "$p"); var pT = P(cmd, "$t"); var pG = P(cmd, "$g"); var pN = P(cmd, "$n");
        foreach (var r in rows)
        {
            pPid.Value = (object?)Long(r, "PID") ?? DBNull.Value; pP.Value = (object?)Str(r, "Process") ?? DBNull.Value;
            pT.Value = (object?)Str(r, "Type") ?? DBNull.Value; pG.Value = (object?)Str(r, "GrantedAccess") ?? DBNull.Value;
            pN.Value = (object?)Str(r, "Name") ?? DBNull.Value;
            cmd.ExecuteNonQuery();
        }
        tx.Commit();
        return rows.Count;
    ...
    private static bool Bool(Dictionary<string, JsonElement> r, string k)
    {
        if (!r.TryGetValue(k, out var v)) return false;
        return v.ValueKind switch
        {
            JsonValueKind.True => true,
            JsonValueKind.False => false,
            JsonValueKind.String => v.GetString() is "True" or "true" or "1",
