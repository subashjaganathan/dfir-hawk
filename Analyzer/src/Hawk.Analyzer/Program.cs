    ...
    private static int RunCli(string[] args)
    {
        switch (args[0].ToLowerInvariant())
        {
            ...
            case "memory":
            {
                ...
                var handles = args.Contains("--handles");
                for (var i = 2; i < args.Length - 1; i++)
                {
                    if (args[i] == "--image") image = args[i + 1];
                    else if (args[i] == "--vol") vol = args[i + 1];
                }
                if (image is null) { Console.Error.WriteLine("usage: hawk memory <hawk.db> --image <physmem.raw> [--vol <cmd>] [--handles]"); return 1; }
                using var conn = Db.Open(args[1]);
                var n = MemoryAnalyzer.Analyze(conn, image, vol, handles, m => Console.WriteLine("    " + m));
